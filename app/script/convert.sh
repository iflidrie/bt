...
function handleZipFile()
{
	...
	grep -rl --include=\*.py --include=\*.sh --include=index.js 'https://bt.drie.cn' /tmp/package | xargs -I @ sed -i "s|https://bt.drie.cn|${SITEURL}|g" @
...
{
	sed -i "s|https://bt.drie.cn|${SITEURL}|g" $1
}
